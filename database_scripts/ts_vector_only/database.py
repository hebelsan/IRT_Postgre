...
##
# ranks pages for search term
##
def db_rank_page(term, save_data):
    conn = None
    try:
        conn = get_connection()
        cur = conn.cursor()
...
##
#  ranks sections for search term
##
def db_rank_sec(term, save_data):
    conn = None
    try:
        conn = get_connection()
        cur = conn.cursor()
...
def print_top10_section_ids(term):
    conn = None
    try:
        conn = get_connection()
        cur = conn.cursor()
        ...
        sql_string = "SELECT \
                        id, \
                        all_tsv, \
                        (ts_rank_cd(array[1., 1., 1., 1.], all_tsv, \
                            to_tsquery('english', %s), 0) / sqrt(num_words)) AS rank \
                      FROM \
                        search_pages, to_tsquery('english', %s) query \
                      WHERE \
                        query @@ all_tsv \
        ...
        for row in res:
            ...
            tsv = row[1]
            rank = row[2]
            print("section_id: " + str(section_id) + " ranking: " + str(rank))
            #print(tsv)
        print("num results: " + str(len(res)))
